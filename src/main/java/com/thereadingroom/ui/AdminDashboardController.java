...
import com.thereadingroom.db.BookDAO.BookRowDTO;
...
public class AdminDashboardController {
    ...
    private void loadBooks() {
        ...
        for (BookRowDTO dto : BookDAO.findBooksForUser(adminId)) {
        ...
        dialog.setResultConverter(button -> {
            if (button == saveButtonType) {
                ...
                if (!title.isEmpty() && !author.isEmpty()) {
                    int adminId = Session.getUserId();
                    BookDAO.addBookForUser(adminId, title, author, listType);
                    reloadFromDb();
                }
            }
            return null;
    ...
    private void onAnalytics() {
        SceneController.goToAdminAnalytics();
    }

    @FXML
    private void onManageUsers() {
        SceneController.goToManageUsers();
    }

    @FXML
    private void onManageDiscussions() {
        SceneController.goToDiscussions();
    }

    @FXML
    private void onManageBooks() {
        SceneController.goToManageBooks();
    }
